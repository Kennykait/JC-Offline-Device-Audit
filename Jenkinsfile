pipeline {
    ...
    environment {
        PYTHON = 'python'
        SCRIPT_DIR = 'python'
        PS_SCRIPT = 'powershell/RunJumpCloudAudit.ps1'
    ...
    stages {
        stage('Checkout Repository') {
            steps {
                git branch: 'jenkins-fork', url: 'https://github.com/Kennykait/JC-Offline-Device-Audit.git'
            }
        }
        ...
        stage('Fetch Ignore List') {
            steps {
                withCredentials([
                    ...
                    bat '''%PYTHON% python/GoogleSheetsIgnoreFetcher.py %GOOGLE_SHEET_ID% "%IGNORE_TAB_NAME%" --creds %GOOGLE_CREDS%'''
                }
            }
        }
        ...
        stage('Fetch Vacation Data') {
            steps {
                withCredentials([
                    ...
                    bat '''%PYTHON% python/HiBobFetcher.py'''
                }
            }
        }
        ...
        stage('Upload to Google Sheets') {
            steps {
                withCredentials([
                    ...
                    bat '''%PYTHON% python/GoogleSheetsUploader.py %GOOGLE_SHEET_ID% "JumpCloud Report %BUILD_NUMBER%" --creds %GOOGLE_CREDS%'''
                }
            }
        }
    }
